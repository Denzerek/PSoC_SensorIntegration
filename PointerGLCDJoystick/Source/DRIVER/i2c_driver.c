...
void i2c_interrupt_eventsCallback(uint32_t event)
{
    ...
    {
        // i2cStatusDecode(CYBSP_I2C_master_get_status());
        // I2CDRIVER_PRINT("I2C hardware ERROR %X",CYBSP_I2C_master_get_status());
        hardwareErrorFlag = true;
        // Cy_SCB_ClearMasterInterrupt();
        // CYBSP_I2C_master_clearInterrupt(CY_SCB_I2C_MASTER_ERR_EVENT);
        // CYBSP_I2C_master_clearInterrupt(CYBSP_I2C_master_get_status());
    }
}

uint32_t checkI2CHardwareErrorStatus()
{
    uint32_t errors = CYBSP_I2C_master_get_status();
...
i2cDriverState_t i2c_readBurst(uint8_t slaveAddress,uint8_t* registerAddressBuffer,uint8_t registerAddressSize,uint8_t* readDataBuffer,uint8_t readSize)
{
    ...
    */
    i2cStatusDecode( CYBSP_I2C_master_write(&transfer));

    /* Wait for transaction completion */
    while (0UL != (CY_SCB_I2C_MASTER_BUSY & CYBSP_I2C_master_get_status()))
    {
    ...
    */
    i2cStatusDecode(CYBSP_I2C_master_read(&transfer));
    /* Wait for transaction completion */
    while (0UL != (CY_SCB_I2C_MASTER_BUSY & CYBSP_I2C_master_get_status()))
    {
        vTaskDelay(10);if(checkI2CHardwareErrorStatus()) return I2C_DRIVER_RW_FAILED;
